...
- GetUniqueWordCount(): Get the total number of unique words (the example above has 14 unique words)

- processPost(): Implement an algorithm to process the post into a data structure

Helpful Hints:

Think about what data structure to store the post so that it is easy to determine the unique word count and the individual word counts
